...
sqldelight {
    databases {
        create("ComfyMobileDb") {
            ...
            // Per-version baselines now live in databases/{1,2,3,4}.db
            // (generated by scripts/regenerate-sqldelight-baselines.py
            // per task #17); SQLDelight produces the head dump as 5.db
            // via schemaOutputDirectory. With both, verifyMigrations
            // bootstraps from each historical version and confirms that
            // applying every subsequent .sqm reaches the head schema —
            // closing the divergence gap ADR-0006 A1 had to defer.
            //
            // Adding a new ALTER-style .sqm in the future means: (a) run
            // the script to mint the next baseline; (b) commit the new
            // .sqm + new baseline + the auto-regenerated head dump.
